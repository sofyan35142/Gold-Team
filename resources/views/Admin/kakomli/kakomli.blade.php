...
<div class="card mb-4">
  <div class="card-datatable table-responsive pt-0">
    ...
    <a href="/index/tambahkakomli" type="button" class="btn rounded-pill btn-primary">Tambah</a>
    <table class="table" id="kakomli">
  <thead>
    <tr>
      <th scope="col">No.</th>
  ...
  <tbody class="table-group-divider">
    <?php $no = 1; ?>
    @foreach ($data as $kakomli)
    <tr>
        <td>{{ $no }}</td>
        <td>{{ $kakomli->nama_guru }}</td>
        <td>{{ $kakomli->jurusan->jurusan }}</td>
        <td>{!! $kakomli->deskripsi !!}</td>
        <td>
            <img src="{{ asset('kakomli/' . $kakomli->foto) }}" alt="" style="width: 80px; height:80px;">
        </td>


        <td>
            <a href="/kakomli/edit/{{ $kakomli->id }}" class="btn btn-warning"><i class="fa-solid fa-pen-to-square"></i></a>
            <a href="#" class="btn btn-danger delete" data-id="{{ $kakomli->id }}" data-kakomli="{{ $kakomli->kakomli }}"><i class="fa-solid fa-trash"></i></a>
        </td>
    </tr>
...
<script>
    $('.delete').click(function() {
        var kakomliid = $(this).attr('data-id');
        var kakomli = $(this).attr('data-kakomli');

        swal({
                title: "Apakah kamu yakin?",
                text: "Kamu akan menghapus kakomli " + kakomli + "",
                icon: "warning",
                buttons: true,
                dangerMode: true,
            })
            .then((willDelete) => {
                if (willDelete) {
                    window.location = "/kakomli/deletekakomli/" + kakomliid + ""
