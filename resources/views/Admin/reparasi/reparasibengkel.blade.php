...
<div class="card mb-4">
  <div class="card-datatable table-responsive pt-0">
    <h5 class="card-header">Data Buku Pedoman Reparasi Bengkel</h5>
    <a href="/index/tambahreparasi" type="button" class="btn rounded-pill btn-primary">Tambah</a>
    <table class="table" id="reparasi">
  <thead>
    <tr>
      <th scope="col">No.</th>
  ...
  <tbody class="table-group-divider">
    ...
    <tr>
        <td>{{ $no }}</td>
        <td>{{ $reparasi->namabuku }}</td>
        <td>{{ $reparasi->link }}</td>
        <td>
            <img src="{{ asset('foto/' . $reparasi->foto) }}" alt="" style="width: 80px; ">
        </td>


        <td>
            <a href="/reparasi/edit/{{ $reparasi->id }}" class="btn btn-warning"><i class="fa-solid fa-pen-to-square"></i></a>
            <a href="#" class="btn btn-danger delete" data-id="{{ $reparasi->id }}" data-reparasi="{{ $reparasi->namabuku }}"><i class="fa-solid fa-trash"></i></a>
        </td>
    </tr>
...
<script>
    $('.delete').click(function() {
        var reparasiid = $(this).attr('data-id');
        var reparasi = $(this).attr('data-reparasi');

        swal({
                title: "Apakah kamu yakin?",
                text: "Kamu akan menghapus reparasi " + reparasi + "",
                icon: "warning",
                buttons: true,
                dangerMode: true,
            })
            .then((willDelete) => {
                if (willDelete) {
                    window.location = "/reparasi/deletereparasi/" + reparasiid + ""
