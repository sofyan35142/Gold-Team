...
<div class="card mb-4">
  <div class="card-datatable table-responsive pt-0">
    <h5 class="card-header">Data Sosial Media</h5>
    <a href="/index/tambahsosmed" type="button" class="btn rounded-pill btn-primary">Tambah</a>
    <table class="table" id="sosmed">
  <thead>
    <tr>
      <th scope="col">No.</th>
  ...
  <tbody class="table-group-divider">
    <?php $no = 1; ?>
    @foreach ($data as $sosmed)
    <tr>
        <td>{{ $no }}</td>
        <td>{{ $sosmed->script_logo }}</td>
        <td>{{ $sosmed->jenis_sosmed }}</td>
        <td>{{ $sosmed->link }}</td>

        <td>
            <a href="/sosmed/edit/{{ $sosmed->id }}" class="btn btn-warning"><i class="fa-solid fa-pen-to-square"></i></a>
            <a href="#" class="btn btn-danger delete" data-id="{{ $sosmed->id }}" data-sosmed="{{ $sosmed->jenis_sosmed }}"><i class="fa-solid fa-trash"></i></a>
        </td>
    </tr>
...
<script>
    $('.delete').click(function() {
        ...
        swal({
                ...
                text: "Kamu akan menghapus Sosial Media " + sosmed + "",
                icon: "warning",
                buttons: true,
                dangerMode: true,
            })
            .then((willDelete) => {
                if (willDelete) {
                    window.location = "/sosmed/deletesosmed/" + sosmedid + ""
