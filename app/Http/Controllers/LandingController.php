...
class LandingController extends Controller
{
    public function beranda()
    {
        ...
        $sosmed = Sosmed::all();
        return view("landingpage.beranda.beranda", compact('blog','alumni','datakepsek', 'total', 'slider', 'sponsor', 'video', 'sosmed', 'keunggulan'));
    }
    ...
    public function ekstra(Request $request)
    {
        ...
        if ($request->filled('judul')) {
            $dataa= ekstra::where('judul', 'LIKE', '%' . $request->judul . '%')
            ->paginate(3);
        }
        return view('landingpage.Data.Ekstrakulikuler', compact('sponsor', 'dataa', 'sosmed'));
    }
    public function prestasi(Request $request)
    {
        $dataa = prestasi::paginate(3);
        $sponsor = sponsor::all();
        $sosmed = Sosmed::all();
        if ($request->filled('nama_lomba')) {
           $dataa = prestasi::where('nama_lomba', 'LIKE', '%' . $request->nama_lomba . '%')
           ->paginate(3);
        }
        return view('landingpage.Data.prestasi', compact('sponsor', 'dataa', 'sosmed'));
    }
    ...
    public function blogselengkapnya(Request $request)
    {
        ...
        // $data = Blog::query();
        if ($request->filled('judul')) {
            $dataa=blog::where('judul', 'LIKE', '%' . $request->judul . '%')
            ->paginate(3);
        }
        if ($request->filled('kategori')) {
            $dataa=blog::where('kategori', '=', $request->kategori)
            ->paginate(3);
        }
        // $data = $data->get();
